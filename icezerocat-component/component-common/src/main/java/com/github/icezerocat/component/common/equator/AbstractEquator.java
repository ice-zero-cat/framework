package com.github.icezerocat.component.common.equator;
...
public abstract class AbstractEquator implements Equator {
    ...
    /**
     * 对比两个对象的指定属性是否相等，为null或""默认相等，默认为两个对象是否 equals
     * <p>
     * 子类可以通过覆盖此方法对某些特殊属性进行比对
     *
     * @param fieldInfo 当前比对属性信息
     * @return 属性是否相等
     */
    protected boolean isFieldEqualsSelective(FieldInfo fieldInfo) {
        // 先判断排除，如果需要排除，则无论在不在包含范围，都一律不比对
        if (isExclude(fieldInfo)) {
            return true;
        }
        ...
        //字段都为null 或 "" 时不比校
        if (StringUtils.isEmpty(fieldInfo.getFirstVal()) && StringUtils.isEmpty(fieldInfo.getSecondVal())) {
            return true;
        }
        return nullableEquals(fieldInfo.getFirstVal(), fieldInfo.getSecondVal());
    }
    ...
    /**
     * 确定是否需要排除这个字段，子类可以扩展这个方法，自定义判断方式
     *
     * @param fieldInfo 字段信息
     * @return boolea-true（需要排除）
     */
